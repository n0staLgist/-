    ...
    <header className="game-header">
      ...
      <div className="game-header__actions">
        ...
        {showSound && <div className="sound-panel">
          ...
          <label>Реплики<input type="range" min="0" max="100" value={Math.round(effectsVolume * 100)} onChange={(event) => onEffectsVolumeChange(Number(event.target.value) / 100)} /></label>
        </div>}
      </div>
    </header>
  );
